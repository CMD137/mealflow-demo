...
- 底部关键操作固定展示：购物车合计、去结算、提交订单、支付宝结账。
...
| 签到信息 | `GET /users/sign` |
| 执行签到 | `POST /users/sign` |
| 提交订单 | `POST /orders/submit`，用户端将选中购物车项转换为 `items` 提交 |
| 订单列表 | `GET /orders` |
| 订单详情 | `GET /orders/{orderId}` |

签到由用户手动触发：`GET /users/sign` 只读取签到状态，`POST /users/sign` 才会写入 Redis Bitmap。Bitmap key 按用户和月份拆分，日偏移为当月日期减一，积分使用独立 Redis 计数键累计。
| 取消订单 | `POST /orders/{orderId}/cancel` |
| 发起支付宝结账 | `POST /payments/{payOrderId}/checkout` |
