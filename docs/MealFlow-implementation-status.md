...
- `meal-infra`：requestId 幂等、Outbox/consumer 语义的基础组件；主线 consumer 模板强制由业务服务提供数据库序列，不再回退到进程内 ID。
...
- 各持久化服务的业务主键由数据库序列表分配，订单和通知消费记录也使用数据库序列，可安全支持多实例；进程内 `IdGenerator` 仅保留给默认构建排除的 `legacy-demo`。
...
- `meal-order` 的支付成功、未支付取消、已支付退款取消和孤儿支付关闭均已拆为数据库 Saga 步骤，带租约、退避与定时恢复；订单最终状态使用条件更新，避免多实例旧请求覆盖新状态。
...
- **Nacos 服务发现/负载均衡真正生效**：网关路由改为 `lb://meal-*`，order/fulfillment/support 内部客户端 `@LoadBalanced` 按服务名调用；多网卡实例注册 IP 钉到 private 网段（10.200.0.0/16 + preferred-networks）；`meal-catalog` 支持 `--scale 2` 多实例，实测双实例注册、请求 200、停掉一个实例后自动故障转移。
